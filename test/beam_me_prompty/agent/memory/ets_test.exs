defmodule BeamMePrompty.Agent.Memory.ETSTest do
  ...
  describe "info/1" do
    test "returns table information", %{context: context, table_name: table_name} do
      ...
      assert info.table_name == table_name
      assert info.capabilities == [:search, :ttl, :pattern_matching]
    end

    test "shows correct size for multiple entries", %{context: context} do
      {:ok, _} = ETS.store(context, "key1", "value1", [])
      {:ok, _} = ETS.store(context, "key2", "value2", [])
      {:ok, _} = ETS.store(context, "key3", "value3", [])
