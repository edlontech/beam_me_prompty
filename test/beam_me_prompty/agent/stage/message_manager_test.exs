defmodule BeamMePrompty.Agent.Stage.MessageManagerTest do
  ...
  describe "format_response/1" do
    test "formats binary response as TextPart" do
      response = "Hello, world!"

      assert {:assistant, [%TextPart{text: "Hello, world!"}]} =
               MessageManager.format_response(response)
    end

    test "formats map with function_call as FunctionCallPart" do
      function_call = %{name: "test_function", arguments: %{"arg1" => "value1"}}
      response = %{function_call: function_call}

      assert {:assistant, [%FunctionCallPart{function_call: ^function_call}]} =
               MessageManager.format_response(response)
    end

    test "formats regular map as DataPart" do
      response = %{"key" => "value", "number" => 42}

      assert {:assistant, [%DataPart{data: ^response}]} =
               MessageManager.format_response(response)
    end

    test "formats list of responses" do
      response = [
        "text message",
      ...
      {:assistant, parts} = MessageManager.format_response(response)

      assert length(parts) == 3
      assert [%TextPart{text: "text message"}] = Enum.at(parts, 0)
      assert [%FunctionCallPart{}] = Enum.at(parts, 1)
      assert [%DataPart{}] = Enum.at(parts, 2)
    end

    test "formats empty list" do
      response = []

      assert {:assistant, []} = MessageManager.format_response(response)
    end
  end
  ...
  describe "normalize_response_for_validation/1" do
    test "returns map as-is" do
      response = %{"key" => "value", "nested" => %{"data" => 123}}

      assert ^response = MessageManager.normalize_response_for_validation(response)
    end

    test "parses valid JSON binary" do
      json_response = "{\"status\":\"ok\",\"count\":5}"

      result = MessageManager.normalize_response_for_validation(json_response)

      assert %{"status" => "ok", "count" => 5} = result
    end

    test "wraps invalid JSON binary in content key" do
      text_response = "This is plain text, not JSON"

      result = MessageManager.normalize_response_for_validation(text_response)

      assert %{"content" => "This is plain text, not JSON"} = result
    end

    test "wraps list in parts key" do
      list_response = ["item1", "item2", %{"key" => "value"}]

      result = MessageManager.normalize_response_for_validation(list_response)

      assert %{"parts" => ["item1", "item2", %{"key" => "value"}]} = result
    end

    test "wraps other types in data key" do
      # Test atom
      assert %{"data" => :atom_value} =
               MessageManager.normalize_response_for_validation(:atom_value)

      # Test number
      assert %{"data" => 42} =
               MessageManager.normalize_response_for_validation(42)

      # Test tuple
      assert %{"data" => {:ok, "result"}} =
               MessageManager.normalize_response_for_validation({:ok, "result"})
    end
  end
